...
@Slf4j
public class GenericLocalDateDeserializer extends StdDeserializer<LocalDate> {


    public GenericLocalDateDeserializer(Class<?> vc) {
        super(vc);
    }
    public GenericLocalDateDeserializer() {
        this(null);
    }
    @Override
    public LocalDate deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException {
        ...
        for (String DATE_FORMAT : GenericConstants.DATE_FORMATS) {
            try {
                return LocalDate.ofEpochDay(new SimpleDateFormat(DATE_FORMAT).parse(date).toInstant().getEpochSecond());
